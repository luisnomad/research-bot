---
source: x-bookmarks
source_id: '1997123833086566408'
author: '@GoogleCloudTech'
url: 'https://x.com/GoogleCloudTech/status/1997123833086566408'
created: '2026-01-14'
triaged: '2026-01-16'
status: approved
confidence: 0.95
has_images: true
---
# Stop repeating context to your AI. 
@agenticamit
 shows how to use local markdow

Stop repeating context to your AI. 
@agenticamit
 shows how to use local markdown files to chain prompts and scaffold apps faster with Gemini CLI.

Watch the breakdown on this week’s #DEVcember livestream → 
https://
goo.gle/44cxoKW

## Triage Notes

**Decision**: Approved  
**Confidence**: 95%  
**Reason**: The content is relevant and valuable for users interested in AI applications.


## Related Notes

- [[testingcatalog-breaking-google-introduced-gemini-enterprise-with-agents]] (71% match)
- [[aaditsh-this-guy-literally-dropped-15-rules-to-master-vibe-coding]] (69% match)
- [[officiallogank-you-can-now-vibe-code-voice-ai-agents-and-experiences-in]] (69% match)
- [[intheworldofai-gemini-3-deep-think-enhancing-gemini-to-become-a-super]] (68% match)
- [[addyosmani-we-released-canvas-in-gemini-this-week-allowing-you-to]] (67% match)
